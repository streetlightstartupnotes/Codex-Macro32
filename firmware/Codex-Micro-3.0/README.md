# Codex Macro32 V3 for Waveshare ESP32-S3-Touch-LCD-1.85B

> 当前公开版本仅适用于 macOS。项目发布与维护：路灯同学创业笔记（https://github.com/streetlightstartupnotes）。

当前固件版本为 **V3.0.4**。把微雪 ESP32-S3-Touch-LCD-1.85B 变成可被 ChatGPT Desktop 识别的便携
Codex Micro 控制器。项目使用 Vendor HID 兼容协议，不是普通键盘快捷键模拟。

## 版本与使用入口

| 入口 | 内容 | 适合场景 |
| --- | --- | --- |
| `flash.command` | 检查代码、从当前源码构建并上传 | 推荐安装方式 |
| `usb-mic/waveshare-1_85b-usb-mic` | V3 默认发布构建：BLE 控制 + USB Audio 麦克风 | 日常使用 |
| `waveshare-1_85b` | 保留 USB Serial/JTAG 的恢复/调试构建，HID 走 BLE | 串口诊断与恢复 |

USB Vendor HID 已移除。所有 Codex 控制命令、状态与 Companion 数据只走 BLE；V3
默认构建把同一根数据线仅用于供电/充电和输入型 USB Audio 麦克风；不暴露 CDC、
USB 键盘、Vendor HID 或扬声器。

## 第三版边界

本目录是独立于第二版的第三版。当前已加入中央额度数字缩小、中央圆环内
电量图标与电量百分比，以及额度环绿色/黄色/红色分档。第二版目录保持原 UI，不随
第三版变化。

USB Audio 构建使用板载 ES7210 双麦克风，向 macOS 提供名为 `Codex Macro32 Mic`
的 48 kHz、16 位、单声道输入设备。它和 BLE Vendor HID 同时运行；Mac 正在请求麦克风
流时，本地任务提示音自动让路，避免声音混入录音。

项目自带 macOS Companion 默认监控这一标准设备名：插线后自动设为系统默认输入，
拔线后恢复此前输入；连接期间手动换麦克风时不会被抢回。逻辑不写死作者路径、
序列号或某台电脑，GUI、命令行与登录自启动方式均通用。

## 已实现功能

- 360×360 首页以第一代稳定布局为基础，放大为 132 半径六键轨道、86 像素圆键、176 像素中央额度环，
  保留原中央圆盘和纯黑背景；`1–6` 使用 16 像素字号，额度数字使用 40 像素字号，
  中文重置时间完整放在环内。
  仍显示 `1–6` 并保留真实状态整键填色，不恢复旧版 `A1–A6`、英文额度占位或滑动手势。
- Agent 状态颜色填充整个圆形按钮，运行、完成、等待、错误、空闲和未分配可直接区分。
- 点击 1–6 立即切换对应 Agent；长按单个 Agent 后，标题、工作区、状态和运行时间显示约 5 秒。
- Agent 详情使用中文界面和专用中文字形；动态标题或工作区只要含有字库不支持的字符，
  整行直接隐藏，不显示 `?` 或方框。
- 主页中央圆环短点会发送当前输入。按住时长用于选择模式：按住约 0.8s 进入六快捷键页，按住约 5s 进入
  四向摇杆遥控页，按住约 10s 重新配对蓝牙，继续按到 20s 恢复出厂。界面出现后
  松手即确认进入该模式；在快捷页或摇杆页单击中央圆环返回主页。快捷页不会自动
  退出，只能单击中间退出；四向摇杆中点单击 1 次退出。
- 遥控使用全屏四向圆环：上方原生摇杆上推创建新任务，下方摇杆下推切换侧边栏；
  右侧前进发送 `ENC_CC`，左侧后退发送 `ENC_CW`，实际行为跟随 Codex 中选择的旋钮
  模式。界面不增加边缘旋钮，也不依赖隐藏滑动或握持旋转来发送命令。
- BOOT 按住/松开触发 Push-to-talk；350 ms 内双击进入免按住录音，再单击停止。
- 六快捷页点击“录音”会进入免按住录音并显示动态波形，按一次 BOOT 停止。
- 录音时进入独占界面，只显示计时和板载麦克风实时波形；Agent、额度环、连接与
  电量信息全部隐藏。波形以屏幕中心为中轴保持柔和动态，并叠加真实声音能量。
- 所有按钮、录音启停、遥控和恢复出厂均无点击音；开机自检、Agent 完成、等待审批
  和异常状态使用不同提示音；需要外壳内已连接扬声器。
- BQ27220 电量读取、BLE 电量上报和断线重新广播。屏幕背光同步 Codex Micro
  设置中的 0–100% 亮度，并跟随 Codex 的自动调暗与恢复事件。
- 周额度快照写入 NVS。蓝牙断开、重配或尚未认证时固定显示“等待连接”，不会用历史
  快照冒充在线；连接并认证但还没有有效额度时显示“未同步额度”。

### V1.1 companion

- 图形设置界面显示连接状态，并提供额度同步间隔、声音开关、最近六个任务预览和
  “立即同步”。真实额度在连接期间始终同步，不提供关闭额度或多主题选择器。
- 通过 Codex app-server 读取额度及任务标题、工作目录、创建/更新时间；数据经现有
  加密 BLE companion characteristic 发送，不读取或保存 Codex 密码、Token 或 API Key。
- V1.1 固件可接收 Agent 元数据；长按 Agent 时显示 companion 提供的标题和工作区。
- 设备固定使用青蓝贾维斯配色；声音开关会控制设备提示音。翻面静音和 companion
  声音开关任一处于静音状态时，设备均保持静音。同步间隔和声音设置会缓存到 NVS。

### QMI8658 IMU

- 检测到有效移动时只请求本地唤醒，不发送按键、命令或其他危险操作。
- Codex 已经自动调暗到 0% 时，单纯移动设备不会强行覆盖它；触摸或按键活动由 Codex 恢复已设置的亮度。
- 屏幕朝下稳定放置约 600 ms 后静音；翻回朝上后解除翻面静音。
- 自动旋转已按用户要求关闭；移动唤醒和翻面静音保留，屏幕保持默认方向，
  触控坐标不变。
- 移动唤醒有消抖、静止重置和冷却，避免持续抖动反复触发。
- 找不到 QMI8658 时仅关闭运动功能，其他功能继续运行。

### 亮度与自动调暗

- 不需要 Companion 参与。ChatGPT Desktop 会通过现有 BLE Vendor HID 灯光报文发送亮度。
- 固件综合 `v.oai.rgbcfg` 中的按键/环境亮度和 `v.oai.thstatus` 中六个 Agent 的亮度，取当前最高值线性映射到 LCD 背光。协议亮度不再参与 Agent 状态判断或 RGB 缩放，避免背光和颜色双重调暗。
- 灯光传输状态与 LCD 使用的 Agent 语义状态分开保存。自动调暗超时后，Desktop 发送全关状态，固件只把 LCD 背光设为 0，保留最后的蓝/绿/黄/红任务状态；使用 Codex Micro 或状态变化时，Desktop 重新发送用户亮度和当前状态。
- 新连接建立时的零值初始化报文不会误把“等待连接”变黑。只有收到完整且可判定的灯光帧后，才启用 Codex 背光控制；断线时回到默认可见亮度。

## Agent 状态的真实边界

屏幕环形颜色来自 ChatGPT Desktop 发送的 Vendor HID 灯光状态。companion 的任务列表
则来自它自己启动的独立 app-server：`thread/list` 能可靠返回标题/首条消息摘要、
工作目录、创建时间和更新时间，但通常不能观察 Desktop 进程内任务的实时运行态。

当协议返回 `notLoaded` 时，GUI 会显示“不可用”，设备上的动态标题行会隐藏，不会根据
更新时间猜测为运行、空闲或已完成。此降级不等同于任务失败，也不会覆盖 HID 已提供的
环形状态颜色。app-server 协议随 Codex 版本演进，升级后如字段变化需同步适配。

## 编译与刷机

需要 Python 3 和 PlatformIO：

```sh
python3 -m pip install platformio
```

### 从源码构建并刷入

最省事的方法是双击 `flash.command`。脚本会自动选择唯一串口；如检测到多个串口，可指定：

```sh
./flash.command /dev/cu.usbmodemNNN
```

稳定版不暴露 CDC 维护串口。开发板只有 BOOT 和 PWR 两颗实体键；刷机前先拔线并按
PWR 确认关机，按住 BOOT 后插入 USB，必要时再短按 PWR 上电，等待约两秒松开 BOOT，
然后运行脚本。脚本仍兼容从早期带维护串口的 V3 自动迁移一次。公开仓库不附带预编译固件。

默认 V3 USB 麦克风构建：

```sh
cd usb-mic
pio run
```

首次构建必须运行这里的默认两阶段命令。第一阶段 `prepare-bluedroid` 生成兼容库，第二阶段
才生成 USB 麦克风固件；不要在全新克隆中直接跳过第一阶段执行单独的 `-e` 环境。

`flash.command` 会只把生成的应用固件写到 `0x10000`，保留 NVS、蓝牙绑定、额度缓存和
Companion 设置。若只需 USB Serial/JTAG 调试，可在上一级目录运行
`pio run -e waveshare-1_85b` 构建恢复/串口版。

首次刷录后，在 macOS 蓝牙设置中配对 `Codex Micro`，重启 ChatGPT Desktop，并允许
“输入监控”。随后在 ChatGPT 的 `Settings > Codex Micro` 配置 Agent、Command 和
方向动作。若系统缓存旧描述符，先忽略该蓝牙设备，重启硬件后重新配对。

## 运行 companion 1.1

macOS 图形界面：

```sh
cd companion
./launch_companion.command
```

首次运行会创建本地 Python 环境并安装轻量 BLE 依赖。建议选择 300–600 秒，即每
5–10 分钟同步一次；默认是 300 秒。额度或任务刚有变化时可点“立即同步”。更短间隔
虽然可配置，但会增加本机 app-server 和 BLE 唤醒次数。

如需纯命令行模式，可在项目根目录运行：

```sh
./run-usage-bridge.command
# 或指定 10 分钟
companion/.venv/bin/python companion/codex_usage_bridge.py --interval 600
```

GUI、命令行 bridge 和登录自启动脚本共用一个进程锁。同一时间只有一个实例会控制
BLE 和默认麦克风；后启动的实例会明确显示占用状态并停止。更详细的安装、设置、
数据来源和协议说明见
[`companion/README.md`](companion/README.md)。

Bridge 只通过 CoreBluetooth 的 `retrieveConnectedPeripheralsWithServices` 查询
macOS 已连接外设，不调用扫描 API。设备未连接时，它不会启动 Codex app-server；
“立即同步”只会排队，待已有连接出现后执行。

## 安全启用 Wi-Fi OTA

默认 V3 构建没有启用 OTA。仓库和默认构建产物不包含 Wi-Fi 或 OTA 凭据，因此不会
连接 Wi-Fi，也不会开放 OTA 端口。需要自行修改构建开关并显式启用时：

```sh
cp include/CodexV11Secrets.example.h include/CodexV11Secrets.h
```

只在本地副本中填写：

- `CODEX_WIFI_SSID`：1–32 字节；
- `CODEX_WIFI_PASSWORD`：8–63 字节，不支持开放网络；
- `CODEX_OTA_PASSWORD`：至少 8 字节，建议使用与 Wi-Fi 不同的随机密码。

`CodexV11Secrets.h` 已被 `.gitignore` 排除，不要提交、打包或分享。OTA 仅使用 STA
模式，调用 `WiFi.persistent(false)`，不把网络凭据写入 NVS。任一配置缺失或不合规时
会保持禁用。ArduinoOTA 的密码认证不是 TLS，只应在可信私有局域网使用。主机名为
`codex-micro-<设备短 ID>.local`，默认端口 `3232`；上传命令和恢复流程见 USB/OTA
专项文档。OTA 模块不接收 Codex 账号、Token、API Key 或密码。

## 操作速查

| 操作 | 行为 |
| --- | --- |
| 点击 1–6 | 立即切换对应 Agent；界面 1–6 对应协议槽位 `AG00`–`AG05` |
| 长按 1–6 | 显示该 Agent 的详情约 5 秒 |
| 中央圆环短点 | 发送当前输入 |
| 中央圆环按住约 0.8s | 进入六快捷键页 |
| 中央圆环按住约 5s | 进入四向摇杆遥控页 |
| 中央圆环按住约 10s | 断开并重新配对蓝牙 |
| 中央圆环持续按住 20s | 清除 NVS、蓝牙绑定、额度缓存和设置后重启 |
| 快捷/摇杆页中央圆环单击 | 返回主页 |
| 快捷页中间单击 | 退出快捷页；页面本身不自动退出 |
| 摇杆中点单击 1 次 | 退出摇杆 |
| 遥控上 / 下 | 原生摇杆新任务 / 侧边栏 |
| 遥控右 / 左 | `ENC_CC` 前进 / `ENC_CW` 后退，行为跟随 Codex 旋钮模式 |
| 按住 / 松开 BOOT | 开始 / 结束 Push-to-talk |
| 350 ms 内双击 BOOT | 进入免按住录音，再单击停止 |
| 快捷页点击“录音” | 进入免按住录音，按一次 BOOT 停止 |
| 移动设备 | 请求本地唤醒，不发送命令；Codex 自动调暗期间不强制改亮度 |
| 屏幕朝下 / 朝上 | 静音 / 解除翻面静音 |
| 斜握或旋转设备 | 不再自动旋转 UI；移动唤醒和翻面静音保留 |

恢复出厂需要持续按住中央圆环 20 秒；约 10 秒起出现警告倒计时。保持到 0 后才会执行清除。

## 硬件与兼容性限制

- 1.85B 的 PWR 键直接连接电源锁存芯片，不进入 ESP32 GPIO；可编程物理键只有
  BOOT（GPIO0）。因此 PWR 可执行硬件电源动作，但不能由固件改造成单独的屏幕休眠键。USB 已插入时会继续由 VBUS 供电，需要真正断电时先拔掉 USB，再短按 PWR。
- 默认 V3 构建把板载麦克风作为 `Codex Macro32 Mic` 上传到 Mac；应用需要在声音输入
  设置中选择它。恢复/串口版不提供 USB Audio。
- 默认 V3 只显示 `Codex Macro32 Mic`，不再提供 CDC 维护串口；Codex 控制、状态和额度
  全部走 BLE。后续刷机需按前文的 BOOT + PWR 步骤进入 ROM 下载模式。
- 提示音需要硬件已连接扬声器；QMI8658 缺失时运动功能会自动停用。
- 固件依赖未公开的 Codex Micro Vendor HID 协议，ChatGPT Desktop 更新后可能需要
  调整。当前仅支持 macOS，Windows 与 Linux 尚未适配和测试。
- 项目不模拟 Work Louder Input 的额外层、原厂升级器或原厂 RGB 灯效。
