...
require_text "$UI" 'if (!state.bleConnected || !state.bleAuthenticated || !state.hidReady)' 'Codex HID 就绪状态优先级丢失'
...
require_text "$BLE" 'state_.hidReady = true;' 'Codex HID RPC 就绪状态丢失'
require_text "$BLE" 'ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED;' 'macOS HOGP 配对所需的加密描述符权限丢失'
require_text "$BLE" 'input_->getDescriptorByUUID(BLEUUID(static_cast<uint16_t>(0x2902)))' 'HID 输入 CCCD 加密权限修复丢失'
forbid_text "$BLE" 'advertising->addServiceUUID(kCompanionServiceUuid);' 'Companion UUID 又干扰 HID 广播身份'
...
forbid_text "$USB_MIC" '#include <USBCDC.h>' '稳定版 USB 麦克风又引入维护串口'
forbid_text "$USB_MIC" 'USBCDC maintenanceSerial' '稳定版 USB 麦克风又注册维护串口'
forbid_text "$USB_MIC" 'enableReboot(true)' '稳定版 USB 麦克风又允许主机信号触发重启'
require_text "$USB_MIC" 'constexpr BaseType_t kCaptureCore = 1;' 'USB 麦克风采集任务又与蓝牙争用 Core 0'
...
forbid_text "$USB_MIC_CONFIG" 'CONFIG_TINYUSB_CDC_ENABLED=1' '稳定版构建又启用 TinyUSB CDC'
require_text "$USB_MIC_BOARD" '-DARDUINO_USB_CDC_ON_BOOT=0' 'USB 麦克风构建又启用 CDC'
require_text "$PROJECT_DIR/src/Display_ST77916.cpp" 'esp_lcd_panel_io_tx_param(panel_io_handle, -1, NULL, 0);' 'LCD DMA 完成前可能回收 LVGL 缓冲区'
forbid_text "$PROJECT_DIR/src/Display_ST77916.cpp" 'test_draw_bitmap(panel_handle);' '生产固件开机又绘制彩条自检'
require_text "$BLE" 'esp_reset_reason()' '固件复位原因诊断丢失'
require_text "$BLE" 'xQueueCreateStatic' 'BLE 写入队列丢失，重活可能重新进入 Bluedroid 回调'
require_text "$BLE" 'processCompanionWrite(write.data, write.length);' 'Companion 数据没有移到主循环处理'
require_text "$BLE" 'processOutput(write.data, write.length);' 'HID RPC 没有移到主循环处理'
require_text "$MAIN" 'codex.poll();' '主循环没有消费 BLE 写入队列'
