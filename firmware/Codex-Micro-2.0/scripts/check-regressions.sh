...
require_text "$UI" '&lv_font_montserrat_48' '中央额度数字没有使用放大字号'
require_text "$UI" 'lv_obj_align(resetLabel, LV_ALIGN_CENTER, 0, 43);' '重置时间未保持在中央环内'
...
forbid_text "$UI" 'sendMomentary("ACT12");' '中央单击发送旧逻辑残留'
...
require_text "$UI" 'if (state.companionReady)' 'Companion 真实连接状态判断丢失'
